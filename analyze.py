...
for t1 in particlePoints.keys():
    ...
    for p1 in points:
        ...
        for _ in xrange(1, 50):
            debugImage = np.zeros((size[0], size[1], 3), dtype=np.uint8)
            inlierCount = 0
            inliers = []
            t = t2 + 1
            x = bX + t * mX 
            ...
            skipCounter = 0
            while x > 0 and x < size[0] and y > 0 and y < size[1] and t < numFrames and skipCounter < 7:
            ...
            skipCounter = 0
            while x > 0 and x < size[0] and y > 0 and y < size[1] and t > 0 and skipCounter < 7:
                if t in particlePoints:
                    cv2.circle(debugImage, (int(x), int(y)), 4, (255, 0, 0))
                    closest, distance = findClosest((x, y), particlePoints[t])
                    if distance < 7:
        ...
        if len(inliers) > 3:
            ...
            skipCounter = 0
            while x > 0 and x < size[0] and y > 0 and y < size[1] and t < numFrames and skipCounter < 3:
                if t in touchedPoints:
            ...
            skipCounter = 0
            while x > 0 and x < size[0] and y > 0 and y < size[1] and t > 0 and skipCounter < 3:
                if t in touchedPoints:
                    closest, distance = findClosest((x, y), touchedPoints[t])
                    if distance < 3:
                        #print "Found Overlapping inlier -!"
